...
# Per-run accumulator of each sub-agent's REAL measured usage, so the workflow span can
# carry a truthful crew total (supervisor + sum of sub-agents) without re-parsing the
# trace file. Each _run_subagent appends its own (input, output, cost); run() seeds it
# with a fresh list per crew run and reads it back to summarize the workflow span. This
# is measured, not invented - it is the same per-agent numbers _bridge stamps.
_AGENT_TOTALS: contextvars.ContextVar = contextvars.ContextVar("agent_totals", default=None)
...
def _start_root_span(name: str, attributes: dict, parent=None):
    """Start a span, either as a new ROOT trace (parent=None) or nested under `parent`.
    ...
    independent trace we detach the ambient context first, so there is no current span to
    inherit, then start the span, then restore the context.

    When `parent` is given (the workflow span), we DO want to nest under it, so we start
    the span with that explicit parent. This is what puts every agent's LLM + tool spans
    inside the one workflow trace, so the workflow trace's tiles aggregate the real crew
    totals. All spans still carry the shared session.id.
    """
    attrs = dict(attributes)
    sid = _SESSION_ID.get()
    if sid:
        attrs.setdefault("session.id", sid)
    # Nest under the given span (no context detach) - it becomes a child in that trace.
    # NOTE: currently unused by the crew (sub-agents run as their OWN root traces to keep
    # the "separate agents grouped by session" model). Kept for callers that explicitly
    # want a child span. The workflow span is a separate SUMMARY trace, not a parent.
    if parent is not None:
        return span_scope(name, attributes=attrs, parent=parent)
...
def _run_subagent(name: str, question: str, tools_list, system_prompt: str) -> str:
    ...
    Each sub-agent starts a fresh ROOT span (its own trace id) via _start_root_span, so
    on the Traccia Traces page it appears as a SEPARATE execution rather than nested
    inside the workflow/supervisor. This is the deliberate "separate agents grouped by
    session.id" model the article is built on: the four agents are independently owned,
    independently operated services, and "Group by session" folds them back into one
    investigation. The workflow span is a separate SUMMARY trace (also sharing the
    session.id), not a parent - so the agents stay their own traces.

    The sub-agent's span is published as the current agent span (so its tools parent
    their live spans under it, keeping tool timing real and each tool attributed to THIS
    agent).
    """
    agent_id, agent_name = AGENTS[name]
    _emit("agent_started", key=name, agent_id=agent_id, agent_name=agent_name)
    # Own root trace (separate-agents model). session.id ties it to the workflow + siblings.
    ...
    try:
        ...
        # Record this sub-agent's real usage into the per-run accumulator so run() can
        # sum a truthful crew total onto the workflow span (measured, not invented).
        totals = _AGENT_TOTALS.get()
        if totals is not None:
            totals.append((inp, out, cost))
        ...
        # Running crew totals SO FAR (this agent included), for the live UI counter that
        # ticks up as each agent finishes. Real, measured - just cumulative.
        so_far = _AGENT_TOTALS.get() or []
        run_in = sum(t[0] for t in so_far)
        run_out = sum(t[1] for t in so_far)
        run_cost = round(sum(t[2] for t in so_far), 8)
        run_calls = len(so_far)
        _emit("agent_finished", key=name, agent_id=agent_id, agent_name=agent_name,
              ...
              tools_called=called,
              # cumulative crew totals so far (live counter)
              running_input_tokens=run_in, running_output_tokens=run_out,
              running_total_tokens=run_in + run_out, running_cost_usd=run_cost,
              running_llm_calls=run_calls)
...
def run(query: str) -> str:
    ...
    # Fresh accumulator for this run's per-agent usage (sub-agents append into it).
    totals_tok = _AGENT_TOTALS.set([])
    # A dedicated WORKFLOW span for the whole crew run, as its OWN separate SUMMARY trace
    # (its own trace id, sharing this run's session.id). It does NOT parent the agents -
    # they stay independent top-level traces (the "separate agents grouped by session"
    # model the article is built on). "Group by session" on the Traces page folds the
    # workflow + the four agents into one investigation. The workflow span is the
    # orchestration "front door": one entry showing the whole run at a glance (which
    # agents ran, crew-total cost/tokens as summary attributes, end-to-end duration).
    # Note: the workflow trace's headline token/LLM-call tiles read 0 because those tiles
    # aggregate CHILD LLM spans, and by design the workflow has none (the tokens live on
    # the agent traces). The crew totals are carried as workflow.* summary attributes and
    # the live UI meter; that is the honest split (orchestration vs generation).
    wf = _start_root_span("investigation_workflow", attributes={
        "span.type": "workflow",
        "workflow.name": "aws_account_investigation",
    ...
    # parent to it. Sub-agents run as their OWN root traces (separate-agents model).
    ...
    # Summarize the workflow span with a TRUTHFUL crew total: supervisor own usage plus
    # each sub-agent's real usage (from the accumulator). No double-count - a supervisor's
    # accumulated_usage is exclusive of sub-agent tokens (verified: probe_doublecount.py).
    sub_totals = _AGENT_TOTALS.get() or []
    _AGENT_TOTALS.reset(totals_tok)
    crew_in = inp + sum(t[0] for t in sub_totals)
    crew_out = out + sum(t[1] for t in sub_totals)
    crew_cost = round(cost + sum(t[2] for t in sub_totals), 8)
    wf.span.set_attribute("session.id", session_id)
    wf.span.set_attribute("workflow.status", "completed")
    wf.span.set_attribute("workflow.agents", delegated)
    wf.span.set_attribute("workflow.agent_count", len(delegated))
    # Crew-level rollup carried as honest workflow.* summary attributes (measured, not
    # invented). The workflow span stays span.type="workflow" and does NOT masquerade as
    # an LLM span - it made no direct model call, its children (the agent traces) did. The
    # dashboard's headline token/LLM-call tiles will therefore read 0 on the workflow
    # trace (they aggregate child LLM spans); the real per-agent numbers live on the agent
    # traces, and the crew total is available here as workflow.* + on the live UI meter.
    # We DO mirror the cost as llm.cost.usd because the cost tile reads that key directly
    # and the crew cost is a true, useful summary; tokens/calls are left as workflow.*
    # only, to avoid stamping fake LLM usage on a non-LLM span.
    llm_calls = 1 + len(delegated)  # supervisor + each sub-agent that ran
    wf.span.set_attribute("workflow.total_input_tokens", crew_in)
    wf.span.set_attribute("workflow.total_output_tokens", crew_out)
    wf.span.set_attribute("workflow.total_tokens", crew_in + crew_out)
    wf.span.set_attribute("workflow.cost.usd", crew_cost)
    wf.span.set_attribute("workflow.llm_calls", llm_calls)
    wf.span.set_attribute("llm.cost.usd", crew_cost)  # cost tile reads this key
    wf.span.set_attribute("span.tags", ["strands", "multi-agent", "workflow", "crew"])
    wf.end()
    print(f"[workflow] crew total: {crew_in} in / {crew_out} out  ->  ${crew_cost}  "
          f"(agents: {delegated})")
